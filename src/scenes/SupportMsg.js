...
class SupportMsg extends Component {
	constructor(props) {
		super(props);
	...
	msgListener(data) {
		if (data["result"] == 1 && data["result_action"] == "list_support_msg_result") {
						...
						<View style={styles.body}>
							{row.file_thumb == null ? (
								...
								<TouchableOpacity onPress={() => this.openImage(row.file_url)}>
									<Image style={styles.messageThumb} source={{ uri: row.file_thumb }} />
								</TouchableOpacity>
							)}
						</View>
					</View>
					...
					<View style={styles.row}>
						<TouchableOpacity onPress={() => this.gotoProfile(this.user)}>
							<View style={styles.avatar}>
								<Image style={styles.avatarImg} source={{ uri: row.profileImage }} />
							</View>
						</TouchableOpacity>
