...
class EventsPast extends Component {
    ...
    componentDidMount() {
        AsyncStorage.getItem("EventsPast", (err, result) => {
            if (!err && result != null) {
    ...
    gotoEventsView(events_id) {
        this.props.navigation.navigate("EventsView", { events_id: events_id, past: 1, events_type: 4 });
    }
            ...
            <View style={styles.container}>
                {this.props.data.row_count == 0 ? (
                    <View style={styles.profileMessage}>
                        ...
                        <Text style={styles.profileMessageHeader}>No past events to show</Text>
                    </View>
                ) : (
                    <FlatList
