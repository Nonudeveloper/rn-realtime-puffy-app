...
class Feed extends Component {
	...
	msgListener(data) {
		...
		if (data["result"] == 0 && data["result_action"] == "get_puffers_feeds_result") {
			this.setState({
				refreshing: false,
				pullRefreshing: false,
				isLoaded: 1
		...
		//append feed images
		if (data["result"] == 1 && data["result_action"] == "get_puffers_feeds_result") {
			...
			if (this.state.pullRefreshing == false && this.state.last_id > 0) {
				...
				if (localData) {
					AsyncStorage.setItem("Feed", localData);
				}
			}
		}
	}
	...
	componentDidMount() {
		AsyncStorage.getItem("Feed", (err, result) => {
			if (!err && result != null) {
